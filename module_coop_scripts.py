   ...
   [
    #this should be set to run once at game_start
     (try_begin),    #set this first as default, then use saved setting
      ...
      (assign, "$coop_set_max_num_players", 32),
      ...
      (assign, "$coop_set_friendly_fire_damage_friend_ratio", 0),
  ...
  ("coop_server_send_admin_settings_to_player",
    [
     (store_script_param, ":player_no", 1),
           ...
           (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_max_num_players, ":max_num_players"),
            ...
            (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_friendly_fire, ":server_friendly_fire"),
            (server_get_melee_friendly_fire, ":server_melee_friendly_fire"),
            (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_melee_friendly_fire, ":server_melee_friendly_fire"),
            (server_get_friendly_fire_damage_self_ratio, ":friendly_fire_damage_self_ratio"),
            (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_friendly_fire_damage_self_ratio, ":friendly_fire_damage_self_ratio"),
            (server_get_friendly_fire_damage_friend_ratio, ":friendly_fire_damage_friend_ratio"),
            (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_friendly_fire_damage_friend_ratio, ":friendly_fire_damage_friend_ratio"),
            (server_get_ghost_mode, ":server_ghost_mode"),
            (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_ghost_mode, ":server_ghost_mode"),
            (server_get_control_block_dir, ":server_control_block_dir"),
            (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_control_block_dir, ":server_control_block_dir"),
            (server_get_combat_speed, ":server_combat_speed"),
            (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_combat_speed, ":server_combat_speed"),
            (server_get_add_to_game_servers_list, ":server_add_to_servers_list"),
            (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_add_to_servers_list, ":server_add_to_servers_list"),
            (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_player_respawn_as_bot, "$g_multiplayer_player_respawn_as_bot"),
            (multiplayer_send_2_int_to_player, ":player_no", multiplayer_event_return_rules, event_return_valid_vote_ratio, "$g_multiplayer_valid_vote_ratio"),
  ...
  ("coop_find_bot_troop_for_spawn",
    [
      ...
      (try_begin),	  
		...
		#### mod begin
		  (party_get_num_companion_stacks,":num_stacks","$coop_cur_temp_party_enemy"),
		  (store_random_in_range,":stack",0,":num_stacks"),
		### mod end
		
          (party_stack_get_troop_id, ":selected_troop", "$coop_cur_temp_party_enemy", ":stack"), #get one troop from each party per cycle
...
#SP ONLY MISC DATA
      ...
      (else_try),
        ...
        (else_try),
          (eq, ":bandit_type", "trp_sea_raider_light"),		##mod edited 08.12.2019 - different troop coused scene bug
...
#### script_coop_read_eq_from_file
#### Save equipment for player troops in file so they will be equipped in multiplayer battle
#### input: troop_no
#### output: reg31,reg32,reg33,reg34,reg35,reg36,reg37,reg38,reg39,reg40,reg41,reg42,reg43,reg44
	 	 
	("coop_read_eq_from_file",
	 [		 
		(store_script_param_1,":troop_no"),
		(assign,reg11,":troop_no"),
		#(call_script,"script_clear_troops_item_slots","trp_added_items_temp"),
		(troop_clear_inventory,"trp_added_items_temp"),
		# (str_store_troop_name,reg20,":troop_no"),
		#(display_message,"@jednostka odczytana {reg20}"),
		(try_begin),
			(neg|is_vanilla_warband),
			(dict_create, ":dict"),
			...
			(assign,":1h_count",10),
			(assign,":2h_count",15),
			(assign,":polearm_count",20),
			(assign,":shield_count",25),
			(assign,":horse_count",30),
			(assign,":helmet_count",35),
			(assign,":armor_count",40),
			(assign,":gloves_count",45),
			(assign,":boots_count",50),
			(assign,":bow_count",55),
			(assign,":xbow_count",60),
			(assign,":arrows_count",65),
			...
			(try_for_range,":slot_no",0,101),
				...
				(try_begin),
				(gt,":item_no",1),
					(item_get_type, ":itm_type", ":item_no"),
					(try_begin),
						(eq,":itm_type",itp_type_one_handed_wpn),
						(lt,":1h_count",15),
							(troop_set_inventory_slot,"trp_added_items_temp",":1h_count",":item_no"),	
							(val_add,":1h_count",1),
				    (else_try),
						(eq,":itm_type",itp_type_two_handed_wpn),
						(lt,":2h_count",20),
							(troop_set_inventory_slot,"trp_added_items_temp",":2h_count",":item_no"),	
							(val_add,":2h_count",1),
					(else_try),
						(eq,":itm_type",itp_type_polearm),
						(lt,":polearm_count",25),
							(troop_set_inventory_slot,"trp_added_items_temp",":polearm_count",":item_no"),	
							(val_add,":polearm_count",1),
				    (else_try),							
						(eq,":itm_type",itp_type_shield),
						(lt,":shield_count",30),
							(troop_set_inventory_slot,"trp_added_items_temp",":shield_count",":item_no"),	
							(val_add,":shield_count",1),
					(else_try),
						(eq,":itm_type",itp_type_horse),
						(lt,":horse_count",35),
							(troop_set_inventory_slot,"trp_added_items_temp",":horse_count",":item_no"),	
							(val_add,":horse_count",1),
					(else_try),
						(eq,":itm_type",itp_type_head_armor),
						(lt,":helmet_count",40),
							(troop_set_inventory_slot,"trp_added_items_temp",":helmet_count",":item_no"),	# we set value of slot 1 to value of the item so we will get item there 
							(val_add,":helmet_count",1),
					(else_try),
						(eq,":itm_type",itp_type_body_armor),
						(lt,":armor_count",45),
							(troop_set_inventory_slot,"trp_added_items_temp",":armor_count",":item_no"),	# we set value of slot 1 to value of the item so we will get item there 
							(val_add,":armor_count",1),
					(else_try),		
						(eq,":itm_type",itp_type_hand_armor),
						(lt,":gloves_count",50),
							(troop_set_inventory_slot,"trp_added_items_temp",":gloves_count",":item_no"),	# we set value of slot 1 to value of the item so we will get item there 
							(val_add,":gloves_count",1),
					(else_try),		
						(eq,":itm_type",itp_type_foot_armor),
						(lt,":boots_count",55),
							(troop_set_inventory_slot,"trp_added_items_temp",":boots_count",":item_no"),	# we set value of slot 1 to value of the item so we will get item there 
							(val_add,":boots_count",1),
					(else_try),	
						(eq,":itm_type",itp_type_bow),
						(lt,":bow_count",60),
						#(is_between,":item_no",bows_begin,bows_end),
							(troop_set_inventory_slot,"trp_added_items_temp",":bow_count",":item_no"),	# we set value of slot 1 to value of the item so we will get item there 
							(val_add,":bow_count",1),
					(else_try),		
						(eq,":itm_type",itp_type_crossbow),
						(lt,":xbow_count",65),
							(troop_set_inventory_slot,"trp_added_items_temp",":xbow_count",":item_no"),	# we set value of slot 1 to value of the item so we will get item there 
							(val_add,":xbow_count",1),
					(else_try),		
						(eq,":itm_type",itp_type_arrows),
						(lt,":arrows_count",70),
							(troop_set_inventory_slot,"trp_added_items_temp",":arrows_count",":item_no"),	# we set value of slot 1 to value of the item so we will get item there 
							(val_add,":arrows_count",1),
					(else_try),		
						(eq,":itm_type",itp_type_bolts),
						(lt,":bolts_count",75),
							(troop_set_inventory_slot,"trp_added_items_temp",":bolts_count",":item_no"),	# we set value of slot 1 to value of the item so we will get item there 
							(val_add,":bolts_count",1),
					(else_try),		
						(eq,":itm_type",itp_type_thrown),
						(lt,":throwings_count",80),
							(troop_set_inventory_slot,"trp_added_items_temp",":throwings_count",":item_no"),	# we set value of slot 1 to value of the item so we will get item there 
							(val_add,":throwings_count",1),
					#(else_try),
					#	(display_message,"@ its not a valid item"),
					(try_end),
				(try_end),
			(try_end),
			(store_sub,reg31,":1h_count",10),	## We substract 10  from slot of last 
			(store_sub,reg32,":2h_count",15),	## 1h item so we will know how many items of given type are in the temp array
			(store_sub,reg33,":polearm_count",20),
			(store_sub,reg34,":shield_count",25),
			(store_sub,reg35,":horse_count",30),
			(store_sub,reg36,":helmet_count",35),
			(store_sub,reg37,":armor_count",40),
			(store_sub,reg38,":gloves_count",45),
			(store_sub,reg39,":boots_count",50),
			(store_sub,reg40,":bow_count",55),
			(store_sub,reg41,":xbow_count",60),
			(store_sub,reg42,":arrows_count",65),
			(store_sub,reg43,":bolts_count",70),
			(store_sub,reg44,":throwings_count",75),
			
			#(assign,"$last_troop_id",":troop_no"),
		#	(display_log_message,"@1h {reg31}  bows{reg40}  arrows{reg42}"),
...
#### script_coop_read_eq_from_file_to_troops
...
#### script_coop_check_player_troops_status
#### copy eq from file to troops
#### input: 
#### output: 
...
#### script_coop_read_eq_from_troop
#### Save equipment for player troops in file so they will be equipped in multiplayer battle
#### input: troop_no
#### output: reg31,reg32,reg33,reg34,reg35,reg36,reg37,reg38,reg39,reg40,reg41,reg42,reg43,reg44
	 
	 
	("coop_read_eq_from_troop",
	 [		 
		(store_script_param_1,":troop_no"),
		#(call_script,"script_clear_troops_item_slots","trp_added_items_temp"),
		(troop_clear_inventory,"trp_added_items_temp"),
		...
		(try_begin),
			...
			#(neg|troop_is_hero,":troop_no"),
				...
				(assign,":1h_count",10),
				(assign,":2h_count",20),
				(assign,":polearm_count",30),
				(assign,":shield_count",40),
				(assign,":bow_count",50),
				(assign,":xbow_count",60),
				(assign,":arrows_count",70),
				(assign,":bolts_count",80),
				(assign,":throwings_count",90),
				## loop for troops
				(try_for_range,":slot_no",0,":cap"),
					(assign,reg12,":slot_no"),
				...
				(try_end),
			(store_sub,reg31,":1h_count",10),	## We substract 10  from slot of last 
			(store_sub,reg32,":2h_count",20),	## 1h item so we will know how many items of given type are in the temp array
			(store_sub,reg33,":polearm_count",30),
			(store_sub,reg34,":shield_count",40),
			(store_sub,reg40,":bow_count",50),
			(store_sub,reg41,":xbow_count",60),
			(store_sub,reg42,":arrows_count",70),
			(store_sub,reg43,":bolts_count",80),
			(store_sub,reg44,":throwings_count",90),
	...
	# ("coop_read_eq_from_file",
	 # [		 
		...
		# (try_begin),
			# (neg|is_vanilla_warband),
			# (dict_create, ":dict"),
			# (dict_load_file, ":dict","@coop_troops",2),
			# loop for player troops
			# (try_for_range,":slot_no",10,41),
				# (assign,reg12,":slot_no"),
				# (dict_get_int,":item_no",":dict","@troop_{reg11}_{reg12}"),
				# (assign,reg13,0),
				# (val_add,reg13,":item_no"),
				# (display_message,"@kurwa to je item {reg13}"),
				# (try_begin),
				# (display_log_message,"@jednostka {reg11}, nrSlota{reg12}, wartoscSlota{reg13}"),
				# (neg|eq,":item_no",-1),
					# (display_message,"@item id {reg13}"),
					# (break_loop),
				# (try_end),
			# (try_end),
			# (dict_free, ":dict"),
...
("cf_check_file_exist",[
	...
	(neg|is_vanilla_warband),
		(dict_create, "$troop_dict"),
		(dict_load_file, "$troop_dict", "@coop_troops", 2), 
		(dict_is_empty, "$troop_dict"),
		(dict_free, "$troop_dict"),
		(assign, reg5, 1),
	(try_end),
...
#script_wse_window_opened	### mod edited 20.09.2018
...
("cf_wse_window_opened", [
	...
	(eq,"$cheat_mode",1),
		...
		(display_message,"@ window_no: {reg0}"),
		
		(eq,":window_no",8),
		    #(set_trigger_result, "prsnt_game_party_window"),		# set the presentation menu to appear instead of default one
			#(set_trigger_result, "prsnt_game_equip_warriors_window"),
]),
