  ...
  # Give some xp to hero parties
   ...
   [      
       (try_for_range, ":troop_no", active_npcs_begin, active_npcs_end),
         ...
         (val_add, ":trainer_level", 5), #average trainer level is 4 for npc lords, worst : 0, best : 8
         (store_mul, ":xp_gain", ":trainer_level", 500), #xp gain in two days of period for each lord, average : 7000.	# mod edited was 1000
       ...
       (try_for_range, ":center_no", walled_centers_begin, walled_centers_end),         
         ...
         (assign, ":xp_gain", 2000), #xp gain in two days of period for each center, average : 3000.
   ...
   [
    ...
    (try_for_range, ":unused", 0, ":consumption_amount"),
      ...
      (try_for_range, ":cur_food", food_begin, food_end),
		...
		(assign, reg1, ":available_food"),
		#(display_message,"@ available food: {reg1}"),		#Debug
		(call_script, "script_cf_player_has_item_without_modifier", ":cur_food", imod_rotten),
		(item_slot_eq,":cur_food", slot_item_is_blocked, 0),		### mod added 22.09.2018
   ...
   #### Mod if center has no lord, freeze building progress
	...
	### to be tested
      ...
      (eq,":cur_improvement", slot_center_has_kings_smithy),
        ...
        (val_add,":builidng_lvl",1),
        (troop_set_slot,":center_lord",slot_troop_built_smithy,":builidng_lvl"),
      (else_try),
      ...
      (eq,":cur_improvement", slot_center_has_kings_armorer),        
        ...
        (val_add,":builidng_lvl",1),
        (troop_set_slot,":center_lord",slot_troop_built_armorer,":builidng_lvl"),
      (else_try),
      ...
      (eq,":cur_improvement", slot_center_has_kings_stables),        
        ...
        (val_add,":builidng_lvl",1),
        (troop_set_slot,":center_lord",slot_troop_built_stables,":builidng_lvl"),
      (else_try),
      ...
      (eq,":cur_improvement", slot_center_has_kings_bowyer),        
        ...
        (val_add,":builidng_lvl",1),
        (troop_set_slot,":center_lord",slot_center_has_bowyer,":builidng_lvl"),
      (try_end),
      #### MOD END
 ...
 # reserved for future use. For backward compatibility, we need to use these triggers instead of creating new ones.
  ...
  (1,
   [
     ...
     (store_add, ":fac_kingdom_9_plus_one", "fac_kingdom_9", 1),

     (try_for_range, ":faction_1", "fac_kingdom_1", ":fac_kingdom_9_plus_one"),
       (try_for_range, ":faction_2", "fac_kingdom_1", ":fac_kingdom_9_plus_one"),
  ...
  (24,
   [
      ...
      (item_set_slot, "itm_raw_grapes", slot_item_food_bonus, 3),
      (item_set_slot, "itm_apples", slot_item_food_bonus, 4), #brought down from 5
      (item_set_slot, "itm_raw_date_fruit", slot_item_food_bonus, 4),  #19.09.2018
	  ...
	  #Alcohol
      (item_set_slot, "itm_wine", slot_item_food_bonus, 8),	## 30.04.2018
      (item_set_slot, "itm_ale", slot_item_food_bonus, 8),	## 30.04.2018
  ...
  # (72,
   # [
    ...
    # (try_begin),
    # (gt,":one_handed_wpf",30),
		# (store_sub,":one_handed_wpf",":one_handed_wpf",2),
		# (troop_set_proficiency,"trp_player", wpt_one_handed_weapon, ":one_handed_wpf"),
    # (try_end),
	
    # (try_begin),
    # (gt,":two_handed_wpf",30),
		# (store_sub,":two_handed_wpf",":two_handed_wpf",2),
		# (troop_set_proficiency,"trp_player", wpt_two_handed_weapon, ":two_handed_wpf"),
	# (try_end),
	
	# (try_begin),	
	# (gt,":polearm_wpf",30),
		# (store_sub,":polearm_wpf",":polearm_wpf",2),
		# (troop_set_proficiency,"trp_player", wpt_polearm, ":polearm_wpf"),
	# (try_end),
	
	# (try_begin),
	# (gt,":xbow_wpf",30),
		# (store_sub,":xbow_wpf",":xbow_wpf",2),
		# (troop_set_proficiency,"trp_player", wpt_crossbow, ":xbow_wpf"),
	# (try_end),
	
	# (try_begin),
	# (gt,":throwing_wpf",30),
		# (store_sub,":throwing_wpf",":throwing_wpf",2),
		# (troop_set_proficiency,"trp_player", wpt_throwing, ":throwing_wpf"),
	# (try_end),
	
	# (try_begin),
	# (gt,":archery_wpf",30),
		# (store_sub,":archery_wpf",":archery_wpf",2),
		# (troop_set_proficiency,"trp_player", wpt_archery, ":archery_wpf"),
	# (try_end),
