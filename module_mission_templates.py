			...
			(try_for_range, ":slot",0,4),
				...
				(gt,":item",-1),
					...
					(eq,":item_type",itp_type_one_handed_wpn),
						...
						(display_message,"@{reg18} slot {reg21} adding {s10}"),
					(else_try),
					...
					(eq,":item_type",itp_type_one_handed_wpn),
						...
						(spawn_item, ":agent_item", -1, 0),
						(display_message,"@Droppping duplicated 1h item: {s10}"),
					(else_try),
					(gt,":duplicated_2h",1),
					(eq,":item_type",itp_type_two_handed_wpn),
						(agent_get_item_slot, ":agent_item",":agent", ":slot"),
						(agent_unequip_item, ":agent", ":agent_item", ":slot"),
						(display_message,"@Droppping duplicated 2h item: {s10}"),
					(try_end),
				(try_end),
...
#-----------------------------------------------------------------
  ...
  (
    ...
    [
      (1, 0, ti_once, [], [
         # (store_current_scene, ":cur_scene"),
          #(scene_set_slot, ":cur_scene", slot_scene_visited, 1),	11.11.2018 mod edited
          (call_script, "script_init_town_walker_agents"),
          (call_script, "script_music_set_situation_with_culture", mtf_sit_travel),
        ]),
      (ti_before_mission_start, 0, 0, [], [(call_script, "script_change_banners_and_chest")]),
  ...
  (
    ...
    [(0,mtef_team_0|mtef_use_exact_number,af_override_horse|af_override_weapons|af_override_head,aif_start_alarmed,4,[itm_staff]),
     (3,mtef_visitor_source|mtef_team_1,af_override_horse,aif_start_alarmed,1,[]),
     ],
    [
      common_inventory_not_available,
...
### MOD END
  ...
  (	### 08.05.2018
    ...
    [
	  ...
	  (ti_on_agent_spawn, 0, 0, [],	### 08.05.2018
       [
         (store_trigger_param_1, ":agent_no"),
         (call_script, "script_agent_reassign_team", ":agent_no"),
...
## MadVader deathcam end
      ...
      (ti_question_answered, 0, 0, [],
	   ...
	   (ti_before_mission_start, 0, 0, [],	### 08.05.2018
       [
         (team_set_relation, 0, 2, 1),
         (team_set_relation, 1, 3, 1),
         (party_clear, "p_routed_enemies"),
      ...
      (1, 4, ti_once, [(main_hero_fallen)],			### 08.05.2018
          [
              (assign, "$pin_player_fallen", 1),
			  (display_message, "@You have fallen in battle!"),
              # (str_store_string, s5, "str_retreat"),
...
## MadVader deathcam end
      ...
      (1, 4, ti_once, [(main_hero_fallen)],
          [
              (assign, "$pin_player_fallen", 1),
			  (display_message, "@You have fallen in battle!"),
              # (str_store_string, s5, "str_retreat"),		26.12.2018
              # (call_script, "script_simulate_retreat", 5, 20, 0),
              # (assign, "$g_battle_result", -1),
              # (set_mission_result,-1),
              # (call_script, "script_count_mission_casualties_from_agents"),
...
##  ),
    ...
    [
	  ...
	  (0, 0, 3, [(this_or_next|key_clicked, key_p),		## mod 24.12.2018
	  ...
	  [    
		...
		#(eq,s0,"@Smieciumur"),
			(try_begin),
			(key_clicked, key_p),
				(agent_play_sound, ":agent_no", "snd_pawiu_yy"),
			(else_try),
			(key_clicked, key_o),
				(agent_play_sound, ":agent_no", "snd_radon_no_debil"),
			(else_try),
			(key_clicked, key_i),
				(agent_play_sound, ":agent_no", "snd_random_sound"),
			(try_end),
		(try_end),
	  ]),
